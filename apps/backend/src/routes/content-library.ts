...
const contentLibraryRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // --- Image library (for avatar photos, HeyGen talking photo, etc.) ---

  /** Get unique tags from all image items */
  fastify.get('/image/tags', async (request, reply) => {
    const items = await db.select({ tags: contentLibraryItems.tags }).from(contentLibraryItems).where(eq(contentLibraryItems.type, 'image'));
    const tagSet = new Set<string>();
    for (const i of items) {
      const tags = (i.tags ?? []) as string[];
      for (const t of tags) if (typeof t === 'string' && t.trim()) tagSet.add(t.trim());
  ...
  /** List image items. Query: ?forSelect=1 (lightweight for dropdown), ?tags=tag1,tag2 */
  fastify.get<{ Querystring: { forSelect?: string; tags?: string } }>('/image', async (request, reply) => {
    const { forSelect, tags } = request.query;
    const tagList = tags ? tags.split(',').map((t) => t.trim()).filter(Boolean) : undefined;

    let items = await db.select().from(contentLibraryItems).where(eq(contentLibraryItems.type, 'image'));
    ...
    if (forSelect === '1') {
      return reply.send(items.map((i) => ({ id: i.id, name: i.name, tags: i.tags ?? [], r2Key: i.r2Key })));
    }

    return reply.send(items);
  });

  /** Upload image items (multipart: one or more files). Names from original filenames. */
  fastify.post('/image', async (request, reply) => {
    ...
    for await (const part of parts) {
      ...
      const name = part.filename.replace(/\.[^.]+$/, '') || part.filename;
      const buffer = await part.toBuffer();
      const mimeType = MIME_MAP[ext.slice(1)] ?? 'image/jpeg';
      ...
      const r2Key = `content-library/image/${item.id}${ext}`;
      try {
        await uploadToR2(r2Key, buffer, mimeType);
        await db.update(contentLibraryItems)
          .set({ r2Key })
    ...
    if (uploaded.length === 0) {
      return reply.status(400).send({ error: 'No valid image files uploaded. Use jpg, jpeg, or png' });
    }

    return reply.status(201).send(uploaded);
  });

  /** Get single image item metadata */
  fastify.get<{ Params: { id: string } }>('/image/:id', async (request, reply) => {
    const { id } = request.params;
    const [item] = await db.select().from(contentLibraryItems).where(
      and(eq(contentLibraryItems.id, id), eq(contentLibraryItems.type, 'image'))
    );
    if (!item) return reply.status(404).send({ error: 'Not found' });
    return reply.send(item);
  });
  ...
  fastify.patch<{ Params: { id: string }; Body: { name?: string; tags?: string[] } }>('/image/:id', async (request, reply) => {
    const { id } = request.params;
    const schema = z.object({
      name: z.string().min(1).optional(),
      tags: z.array(z.string()).optional(),
  ...
  /** Delete image item + R2 object */
  fastify.delete<{ Params: { id: string } }>('/image/:id', async (request, reply) => {
    const { id } = request.params;
    const [item] = await db.select().from(contentLibraryItems).where(
      and(eq(contentLibraryItems.id, id), eq(contentLibraryItems.type, 'image'))
    );
  ...
  /** Stream/download image file (for preview) */
  fastify.get<{ Params: { id: string } }>('/image/:id/file', async (request, reply) => {
    const { id } = request.params;
    const [item] = await db.select().from(contentLibraryItems).where(
      and(eq(contentLibraryItems.id, id), eq(contentLibraryItems.type, 'image'))
    );
