...
const workflowsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  /** Test crop on a single frame */
  fastify.post<{
    Params: { id: string };
    Body: { projectId: string; videoId: string; x: number; y: number; width: number; height: number; time?: number };
  }>('/:id/test-crop', async (request, reply) => {
    const { projectId, videoId, x, y, width, height, time = 0 } = request.body;
    ...
    // Use ffmpeg to grab one frame with crop
    const { spawn } = await import('child_process');
    const chunks: Buffer[] = [];
    
    // ffmpeg -ss <time> -i <url> -vf crop=w:h:x:y -vframes 1 -f image2 pipe:1
    const args = [
      '-ss', String(time),
      '-i', sourceUrl,
